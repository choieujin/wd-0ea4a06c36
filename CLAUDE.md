...
- 하객 폼은 **닉네임(선택) + 메시지** 를 받는다(비밀번호 없음). 이름·비밀번호는 백엔드에서 선택 사항이고,
  비밀번호를 받지 않으면 본인 삭제가 불가하다(시트에서 행 삭제). 이 규칙을 바꾸면 `apps-script/Code.gs` 와
  `server.mjs` 의 검증도 함께 맞춰야 한다.
- 닉네임을 비워 두거나 입력칸이 없을 때 `guestbook.js` 는 이름 자리에 `ANON_NAME`("하객"), 비밀번호 자리에
  난수 4자리를 보낸다. 시트에 배포된 스크립트가 옛 버전(이름·비밀번호 필수)이어도 통과시키기 위한 것이다.
- 이름이 없거나 `ANON_NAME` 인 글도 카드에 "하객" 으로 **옅게 표시한다**(`.gb-item__name--anon`).
  닉네임이 있는 글과 카드 모양을 맞춰 옛 익명 글이 섞여도 어색하지 않게 하기 위한 것이다.
